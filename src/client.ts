...
export class HttpClient {
  ...
  constructor(apiKey: string, baseUrl?: string) {
    if (!apiKey.startsWith("sk_live_")) {
      throw new PulseError('Invalid API key format. Keys must start with "sk_live_"');
    }
    this.apiKey = apiKey;
  ...
  private handleError(
    ...
    if (status === 402 && errorCode === "credit_exhausted") {
      throw new PulseCreditExhaustedError(message);
    }
