...
// Raw fetch helper for billing endpoints (not yet in SDK)
async function billingRequest(method: string, path: string, body?: unknown) {
  ...
  if (!res.ok) {
    throw new Error(`Billing API ${res.status}: ${JSON.stringify(json)}`)
  }
  return json
}
...
async function main() {
  ...
  console.log(`Meter created: ${tokenMeter.id} (${tokenMeter.displayName}) — $${tokenMeter.unitPrice}/token`)
  ...
  console.log(`Meter created: ${requestMeter.id} (${requestMeter.displayName}) — $${requestMeter.unitPrice}/req`)
  ...
  const totalCost = usageItems.reduce((sum: number, item: any) => sum + parseFloat(item.totalAmount), 0)
  ...
  const invoice = await billingRequest('POST', `/products/${product.id}/invoices`, {
  ...
  if (invoice.paymentLinkId) {
    const sendResult = await billingRequest(
      'POST',
      `/products/${product.id}/invoices/${invoice.id}/send`
    )
